...
The solution is as simplistic as possible while also trying
to comply to the best design practices. A UML Class Diagram depicting
all implemented classes and interfaces is shown below.

image:ClassDiagram.png[Class Diagram]

The `App` class is the entrypoint of the application and is used to run it.
The `IAnalysis` interfaces defines what an analysis does and can be implemented
by other classes. Currently only the ``TableAnalysis`` implements the interface
for the purpose of analyzing tables, however, further classes for other types of
analysis, for example JSON files, can be easily added.
The ``IReader`` interfaces defines the functionality of the readers. Currently,
there is only a CSV file reader, but additional ones can be easily added.
The ``IValueSearch`` defines common behavior used by the classes responsible for the
search of the respective values. Currently, two types of search are supported, one for
the lowest difference between two values, and another one for the lowest absolute difference.
Further documentation is provided in the comments of the respective classes and methods.
